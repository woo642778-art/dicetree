  ...
  return <div className="build-manager">
    ...
    {shareUrl && <div className="build-popover" data-testid="share-popover">
      <div className="popover-head">
        <strong>{t("action.share")}</strong>
        <button type="button" onClick={() => setShareUrl("")} aria-label={t("action.close")}>×</button>
      </div>
      <p className="muted">{copied ? t("action.copied") : t("share.ready")}</p>
      <div className="save-row">
        <input readOnly value={shareUrl} data-testid="share-url" aria-label={t("share.url")} onFocus={(event) => event.currentTarget.select()} />
        <button type="button" onClick={() => copyUrl(shareUrl)}>{t("action.copy")}</button>
      </div>
