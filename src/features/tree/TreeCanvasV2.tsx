...
const ICON_GLYPHS: Array<[RegExp, string]> = [
  [/snow/i, "✦"], [/vortex/i, "◉"], [/yinyang/i, "◒"], [/gear/i, "⚙"],
  [/lightning|bolt/i, "ϟ"], [/target/i, "◎"], [/clover/i, "✣"], [/speed/i, "≫"],
  [/flower/i, "✹"], [/clock/i, "◷"], [/curve|spiral/i, "↻"], [/star/i, "✦"],
  [/character/i, "◆"], [/tiles/i, "▦"], [/arrows/i, "↗"], [/frame/i, "▣"],
];
...
export function TreeCanvasV2({
  ...
  const wheel = (event: WheelEvent<SVGSVGElement>) => {
    ...
    setView((current) => ({ ...current, scale: Math.max(0.38, Math.min(2.4, current.scale * factor)) }));
  };

  const pointerDown = (event: PointerEvent<SVGSVGElement>) => {
    if ((event.target as Element).closest("[data-tree-node='true']")) return;
  ...
  const zoom = (delta: number) => setView((current) => ({ ...current, scale: Math.max(0.38, Math.min(2.4, current.scale + delta)) }));

  return <div className="tree-canvas-wrap">
    <svg
      className="tree-canvas-v2"
      data-testid="tree-canvas"
      viewBox="-720 -920 1440 1840"
      role="tree"
    ...
    >
      <defs>
        <filter id="soft-shadow" x="-60%" y="-60%" width="220%" height="220%">
          <feDropShadow dx="0" dy="5" stdDeviation="7" floodColor="#392f7d" floodOpacity="0.16" />
        </filter>
        <filter id="active-glow" x="-80%" y="-80%" width="260%" height="260%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>
        <linearGradient id="recommended-line" x1="0" x2="1">
          <stop offset="0" stopColor="#765ce9"/><stop offset="1" stopColor="#d8a83d"/>
        </linearGradient>
      </defs>
      <g data-testid="tree-transform" transform={`translate(${view.x / view.scale} ${view.y / view.scale}) scale(${view.scale})`}>
        ...
        {nodes.map((node) => {
          ...
          return <g
            key={node.id}
            className={`node-token-v2 ${confidenceClass(node.fieldConfidence.effect ?? "unknown")} ${selected ? "is-selected" : ""} ${planned ? "is-planned" : ""} ${recommended ? "is-recommended" : ""} ${dim ? "is-dim" : ""}`}
          ...
          >
            ...
            {square
              ? <rect className="token-shell" x={-radius} y={-radius} width={radius * 2} height={radius * 2} rx={node.kind === "core" ? 15 : 10} />
              : <circle className="token-shell" r={radius} />}
            {square
              ? <rect className="token-face" x={-radius + 5} y={-radius + 5} width={(radius - 5) * 2} height={(radius - 5) * 2} rx={8} />
              : <circle className="token-face" r={radius - 5} />}
    ...
    <div className="canvas-controls" aria-label={locale === "ko" ? "트리 보기 조절" : "Tree view controls"}>
      ...
      <button type="button" className="fit-control" onClick={fit}>{locale === "ko" ? "전체" : "Fit"}</button>
