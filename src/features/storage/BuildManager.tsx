...
export function BuildManager({ state, onLoad }: { state: PlannerStateV1; onLoad: (state: PlannerStateV1) => void }) {
  ...
  const save = () => {
    ...
    saveNamedBuild(trimmedName, state);
    setName("");
    refresh();
  };
  return <div className="build-manager">
    ...
    {open && <div className="build-popover">
      ...
      <div className="save-row"><input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("save.name")} /><button type="button" onClick={save} disabled={!trimmedName}>{t("action.save")}</button></div>
