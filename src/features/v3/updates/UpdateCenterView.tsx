...
export function UpdateCenterView({ data, locale, activeDeckIds }: { data: CanonicalGameData; locale: "ko" | "en"; activeDeckIds: string[] }) {
  ...
  return <main className="v47-update-center" data-testid="v47-update-center">
    ...
    <section className="v47-patch-impact" data-testid="v47-patch-impact">
      ...
      {!impact ? <div className="v47-impact-empty"><strong>{locale === "ko" ? "이전 클라이언트 비교 데이터가 아직 없습니다." : "No previous-client comparison is loaded."}</strong><p>{locale === "ko" ? "현재 v1.0.1 데이터만으로는 변경 전후 수치를 만들 수 없습니다. 공식 공지의 ‘오류 수정 및 최적화’를 밸런스 패치로 확대 해석하지 않습니다." : "Only v1.0.1 data is available, so before/after values cannot be constructed. The generic store note is not treated as a balance patch."}</p></div> : <div className="v47-impact-grid">
