...
export function StatPanel({ result, locale }: StatPanelProps) {
  ...
  return <section className={`v3-stat-panel is-${result.simulation.confidence}`} data-testid="v3-stat-panel">
    <header>
      <div><small>{locale === "ko" ? "실전 DPS" : "Practical DPS"}</small><strong>{number(result.simulation.practicalDps, 2)}</strong></div>
    ...
    <div className="v3-stat-grid">
      {Object.entries(stats).filter(([key]) => key in STAT_LABELS).map(([key, value]) => <div key={key}>
        <span>{STAT_LABELS[key][locale]}</span><strong>{number(value)}</strong>
      </div>)}
      <div><span>{locale === "ko" ? "초당 공격 횟수" : "Attacks / sec"}</span><strong>{number(attacksPerSecond)}</strong></div>
      <div><span>{locale === "ko" ? "기본 공격 DPS" : "Basic attack DPS"}</span><strong>{number(result.simulation.basicAttackDps, 2)}</strong></div>
    </div>
