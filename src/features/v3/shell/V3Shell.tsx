  ...
  return <div className={`v3-app v41-mode-${tab}`} data-testid="v3-app">
    <header className="v3-header">
      ...
      <div className="v3-header-actions">
        <button className="v53-desktop-credit" type="button" onClick={() => setAboutOpen(true)}>{locale === "ko" ? "제작자 모님" : "Created by Monim"}</button>
    ...
    {headerMenuOpen && <div className="v53-header-menu" role="menu">
      ...
      <button role="menuitem" type="button" onClick={() => { setHeaderMenuOpen(false); setAboutOpen(true); }}>{locale === "ko" ? "제작자 및 사이트 정보" : "Creator and site info"}</button>
    ...
    {aboutOpen && <div className="v53-sheet-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setAboutOpen(false); }}><section className="v53-action-sheet v54-about-sheet" role="dialog" aria-modal="true" aria-labelledby="v54-about-title"><div className="v53-sheet-handle" /><header><div><small>DICETREE · COMMUNITY TOOL</small><h2 id="v54-about-title">{locale === "ko" ? "제작자 모님" : "Created by Monim"}</h2></div><button autoFocus type="button" aria-label={locale === "ko" ? "사이트 정보 닫기" : "Close site info"} onClick={() => setAboutOpen(false)}>×</button></header><div className="v54-about-mark" aria-hidden="true"><b>RD</b><i>2</i></div><p className="v54-about-intro">{locale === "ko" ? "게임 안에서 재화를 쓰기 전에 다이스 트리 투자, 덱 구성, 전투 결과를 안전하게 검토할 수 있도록 만든 랜덤 다이스 2 비공식 팬 도구입니다." : "An unofficial Random Dice 2 fan tool for reviewing Dice Tree investments, deck composition, and combat results before spending resources in game."}</p><div className="v54-about-principles"><article><strong>{locale === "ko" ? "근거를 구분합니다" : "Evidence stays visible"}</strong><span>{locale === "ko" ? "검증된 계산, 부분 검증, 추정을 결과마다 표시합니다." : "Every result distinguishes verified, partial, and estimated calculations."}</span></article><article><strong>{locale === "ko" ? "사용자 선택을 우선합니다" : "Your choices come first"}</strong><span>{locale === "ko" ? "추천 이유와 비용을 보여주고 사용자가 경로를 다시 조정할 수 있게 합니다." : "Recommendations expose their reasons and costs so routes remain editable."}</span></article></div><a className="v54-about-link" href="https://github.com/woo642778-art/dicetree" target="_blank" rel="noreferrer">{locale === "ko" ? "GitHub에서 DiceTree 보기" : "View DiceTree on GitHub"}</a><small className="v54-about-disclaimer">{locale === "ko" ? "DiceTree는 게임 제작사와 제휴되지 않은 비공식 팬 프로젝트입니다." : "DiceTree is an unofficial fan project and is not affiliated with the game's publisher."}</small></section></div>}
