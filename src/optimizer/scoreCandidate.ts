import type { DiceDefinition, PlannerStateV1, RecommendationScore, TreeNodeDefinition, VerificationStatus } from "../domain/types";
...
export function scoreCandidate(
  ...
  const costs = calculateRouteCost([{ nodeId: node.id, targetRank: nextRank }], state.ranks, [node]);
  const normalizedCost = costs.gold / 1000 + costs.core * 80 + costs.token * 35;
